...
public final class Signature {
    ...
    public static String sha1(String text) {
        return sign(text, Charset.UTF8, EncryptType.SHA1);
    }

    public static String md5(String text) {
        return sign(text, Charset.UTF8, EncryptType.MD5);
    }

    public static String sign(String text, Charset charset, EncryptType type) {
        try {
            // 获得MD5摘要算法的 MessageDigest 对象
            MessageDigest msgDigest = MessageDigest.getInstance(type.type);
            // 使用指定的字节更新摘要
            msgDigest.update(text.getBytes(charset.getValue()));
            // 获得密文,把密文转换成十六进制的字符串形式
            return ByteExtend.byteArrayToHex(msgDigest.digest());
    ...
    public static Boolean equalsSHA1(String original, String ciphertext) {
        return equals(original, ciphertext, Charset.UTF8, EncryptType.SHA1);
    }

    public static Boolean equalsMD5(String original, String ciphertext) {
        return equals(original, ciphertext, Charset.UTF8, EncryptType.MD5);
    }

    public static Boolean equals(String original, String ciphertext, Charset charset, EncryptType type) {
        return Objects.nonNull(original) && Objects.nonNull(ciphertext) && ciphertext.equals(sign(original, charset, type)) ? Boolean.TRUE : Boolean.FALSE;
    }
}